...
subtitle: "Modelización del segundo nivel de la jerarquía"
output: 
    html_document: 
      code_download: true 
      number_sections: yes
...
Hay 9 categorías pero varias con muy pocas observaciones. En lo que
sigue, exploraremos diferentes formas de definir mejor estas
clasificaciones
...
Hacemos lo que hemos hecho hasta ahora: Seleccionamos todas las
categorías minoritarias y las agrupamos bajo "otros". Sabemos que esto
no necesariamente es lo mejor, pero nos puede dar una idea.

Note que lo que hacemos es redefinir el tipo para que ahora contenga el
nombre de la serie documental

```{r}
otros <- c("400-CONSTRUCCION","500-DIRECCION CALIDAD, AMBIENTE Y PREVENCION DE RIESGO",
          "Actas de Cierre y entrega de las Unidades Funcionales", "Planes de Obras",
          "Proceso de Adquisicion","Programas de Seguridad y Salud en el Trabajo")
datos_400 <- datos_400 %>%
  mutate(
    tipo = factor(if_else(
...
Ahora procedemos como usual.

Primero, hacemos la división

```{r}
division <- initial_split(datos_400, strata = tipo)
...
write_csv(final_res_metrics,"ficha_metadatos_2nivel.csv")

```

Veamos si estos resultados se pueden mejorar usando otro tipo de
clasificación previa de `nombre_serie_documental`

# Segunda forma

En vez de clasificar como "otros", vamos a omitir las clases
minoritarias y obtenemos la clasificación solo para las restantes

```{r}
datos_400_otra <- datos %>% 
  filter(tipo == 400) %>% 
  filter(!(nombre_serie_documental %in% c("400-CONSTRUCCION",
                                          "500-DIRECCION CALIDAD, AMBIENTE Y PREVENCION DE RIESGO",
                                          "Actas de Cierre y entrega de las Unidades Funcionales",
...
# Convertimos a tipo
datos_400_otra <- datos_400_otra %>%
  mutate(
    tipo = factor(nombre_serie_documental)
    )
...
```{r}
division_otra <- initial_split(datos_400_otra, strata = tipo)
entrenamiento_otra <- training(division_otra)
prueba_otra <- testing(division_otra)
pliegos_otra <- vfold_cv(entrenamiento_otra,strata = tipo)
```
...
correlacion_matthews_otra <- mcc(final_res_predictions_otra,truth = tipo, estimate = .pred_class)
final_res_metrics_otra <- bind_rows(final_res_metrics_otra, correlacion_matthews_otra) %>%
  select(-.config)

final_res_metrics_otra
write_csv(final_res_metrics,"ficha_metadatos_2nivel_otra.csv")

```

# A manera de conclusiones 

En la reunión se decidió elimnar de consideración los tipos que ocurren por
debajo de un umbral dado. Eso es lo que se ensaya en el segundo documento de
esta serie. 
